...
// Simple rule-based chatbot steps for complaint filing
const steps = [
...
// POST /chatbot/next-step - get next chatbot step based on current step index
router.post('/next-step', (req, res) => {
  const { stepIndex } = req.body;
  if (stepIndex === undefined || stepIndex < 0 || stepIndex >= steps.length) {
    return res.status(400).json({ error: 'Invalid step index' });
  }
  res.json({ message: steps[stepIndex] });
});
